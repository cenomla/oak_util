...
int main(int, char**) {
	...
	fmtStr = oak::fmt(&oak::temporaryMemory, "%b %o %x", 59, 59, 59);
	oak::print_fmt(fmtStr);
	oak::print_fmt("\n");
	assert(fmtStr == "0b111011 073 0x3b");

	fmtStr = oak::fmt(&oak::temporaryMemory, "%b", static_cast<i64>(0xFFFFFFFFFFFFFFFF));
	oak::print_fmt(fmtStr);
	oak::print_fmt("\n");
	assert(fmtStr == "-0b1");

	fmtStr = oak::fmt(&oak::temporaryMemory, "%b", 0xFFFFFFFFFFFFFFFF);
	oak::print_fmt(fmtStr);
	oak::print_fmt("\n");
